...
func AddDevDomain() {
	...
	f, err := os.OpenFile("/etc/hosts", os.O_RDWR|os.O_APPEND, 0644)
	if err != nil {
		ui.LogFatal("[utils/hostfile] os.OpenFile() failed", err)
	}
	defer f.Close()
	...
	if _, err := f.WriteString(entry); err != nil {
		ui.LogFatal("[utils/hostfile] WriteString() failed", err)
	}
...
func RemoveDevDomain() {
	...
	if err := ioutil.WriteFile("/etc/hosts", []byte(contents), 0644); err != nil {
		ui.LogFatal("[utils/hostfile] ioutil.WriteFile failed", err)
	}
