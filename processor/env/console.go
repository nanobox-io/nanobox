...
func (envConsole processEnvConsole) Process() error {
	...
	//
	id := fmt.Sprintf("nanobox-%s-%s-%s", config.AppName(), envConsole.control.Env, envConsole.container)
...
func (envConsole *processEnvConsole) validateMeta() error {
	...
	if envConsole.container == "" {
		envConsole.container = fmt.Sprintf("nanobox-%s-build", config.AppName())
	}
