...
func (c *ServiceRestartCommand) Run(fApp string, opts []string, api *pagodaAPI.Client) {
	...
	if _, err := api.RestartAppService(fApp, service.ID); err != nil {
		fmt.Println("There was a problem restarting %s. See ~/.pagodabox/log.txt for details", fService)
		ui.Error("pagoda service:restart", err)
	}

	fmt.Printf("Restarting %s's %s (%s)... Check your dashboard for transaction details and logs.\n", fApp, service.UID, service.Name)
}
